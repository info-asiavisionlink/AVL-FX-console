...
//|                  AVL Console — Market Data Manager EA v1        |
//|                                                                  |
//| ⚠️ NO TRADE EXECUTION — データ収集専用                           |
//|   このEAは取引注文を一切送信しません。                           |
//|   BUY / SELL / CLOSE / MODIFY 機能は実装されていません。        |
//|                                                                  |
//| 設計原則                                                         |
...
//|   Stream 2: OHLC Stream     — OnTick() 毎（全8時間足）          |
//|   Stream 3: MarketWatch     — OnTimer() 3秒毎（全シンボル）      |
//|   Stream 4: Indicators      — OnTimer() 30秒毎（拡張インジケーター）|
//|   Stream 5: Orders          — OnTimer() 5秒毎（監視のみ / 注文不可）|
...
void OrderStream_Send()
{
   ...
   // ⚠️ 注文ポーリング無効 — Console DataManagerは取引しません
}
